...
{
	if (num_of_prop == 0 && s == 8) return 0;
	if (num_of_prop == 0 && s == 6) return 1;
	if (num_of_prop == 0 && s == 3) return 2;
...
{
	if (num_of_prop == 0 && o == 0) return 8;
	if (num_of_prop == 0 && o == 1) return 6;
	if (num_of_prop == 0 && o == 2) return 3;

	return 0;
}
void transition0(bool a, bool b, fsm_state input_state, __global fsm_state* output_state)
{	//property "a U b"

	//transition branches
	if( input_state == 8 && ( a && b ))
		*output_state = 8;
	if( input_state == 8 && ( a && ! b ))
		*output_state = 8;
	if( input_state == 8 && ( ! a && b ))
...
prop0: 
	...
	for (int j=(global_id*work_per_thread); j<((global_id+1)*work_per_thread); ++j)
	{
		...
		transition0(a, b, 6, &(output_buffer[nitems*0 + 0 + j*1]));
	}
