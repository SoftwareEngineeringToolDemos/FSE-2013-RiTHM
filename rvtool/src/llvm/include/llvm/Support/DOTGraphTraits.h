...
public:
  ...
  /// ----------------------- ADDED BY SAM
  /// getlines - Given a node and a pointer to the top level graph, return
  /// the lines to print in the node.
  template<typename GraphType>
  std::string getNodeLines(const void *Node, const GraphType& Graph) {
    return "";
  }
  /// ----------------------- ADDED BY SAM	
